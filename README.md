...
### 최근 UI/UX 개선사항 (2025-08-04)

- **완벽한 다크모드 지원**: 업로드 화면, 에디터, 문서 목록 모두 다크 테마 적용
- **2행 도구 팔레트**: 모바일에서 모든 도구를 효율적으로 표시
- **향상된 가독성**: 48x48px 터치 타겟, 6x6 아이콘 크기, 채워진 색상 박스
- **모던 디자인**: 그라디언트 효과, 그림자, rounded-xl 적용
- **액션 버튼 차별화**: Undo/Redo는 회색, Delete는 빨간색 테마
...
모든 페이지에서 일관된 네비게이션 경험을 제공하는 통합 헤더 시스템입니다.

### 주요 특징

- **Breadcrumb 네비게이션**: 현재 페이지의 계층 구조를 명확히 표시
- **컨텍스트 액션 버튼**: 페이지별 맞춤 액션 버튼 제공
- **반응형 디자인**: 모바일에서는 텍스트를 아이콘으로 자동 변환
- **접근성 지원**: WCAG 표준 준수, 스크린 리더 완벽 지원
- **건설 현장 최적화**: 장갑 모드 등 3가지 터치 모드 지원
...
<PageHeader
  title="문서함"
  actions={[
    { label: '새 문서', icon: Plus, href: '/documents/new' },
...
## 보안 및 권한 관리 (2025-08-07 업데이트)

### 계층적 권한 시스템

Row Level Security (RLS)를 통한 데이터 접근 권한 관리:
...
#### 👔 관리자/현장관리자 (admin, site_manager)  
- 배정된 현장의 모든 데이터 접근
- 같은 현장 팀원들의 프로필 조회
- 현장별 데이터 관리 권한

#### 👷 일반 작업자 (worker)
- 본인 데이터 접근 (출근기록, 작업일지 등)
- 같은 현장 팀 데이터 조회 권한
- 현장별 데이터 격리 유지

### 주요 보안 기능

- **무인증 접근 차단**: 로그인하지 않은 사용자는 데이터 접근 불가
- **현장별 데이터 격리**: 다른 현장의 데이터는 완전히 차단
- **프로필 자동 생성**: 새 사용자 로그인 시 역할별 프로필 자동 생성
- **무한 재귀 방지**: RLS 정책의 순환 참조 방지 로직 적용
...
| 이메일 | 비밀번호 | 역할 | 설명 |
|--------|---------|------|------|
| worker@inopnc.com | password123 | worker | 일반 작업자 |
| manager@inopnc.com | password123 | site_manager | 현장 관리자 |
| customer@inopnc.com | password123 | customer_manager | 파트너사 |
| admin@inopnc.com | password123 | admin | 관리자 |
| production@inopnc.com | password123 | site_manager | 생산 관리자 |
...
### 데이터가 표시되지 않는 경우
1. RLS 정책 확인 (사용자 역할과 현장 배정 확인)
2. 데이터베이스 마이그레이션 실행 여부 확인
3. 사용자의 site_assignments 테이블 확인
