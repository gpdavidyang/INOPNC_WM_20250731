...
export default function AdminPermissionValidator({ profile }: AdminPermissionValidatorProps) {
  ...
  const initialTests: TestResult[] = [
    ...
    // Salary Management Tests
    {
      id: 'salary-rules',
      name: '급여 규칙 관리',
      description: '급여 계산 규칙을 조회하고 관리할 수 있는지 테스트',
      status: 'pending',
      category: 'crud'
    },
    {
  ...
  const runTest = async (testId: string): Promise<TestResult> => {
    ...
    try {
      switch (testId) {
        ...
        case 'salary-rules':
          const salaryResult = await getSalaryRules(1, 5)
          if (salaryResult.success) {
            return {
              ...test,
              status: 'passed',
              message: `급여 규칙 ${salaryResult.data?.total || 0}개 조회 성공`,
              duration: Date.now() - startTime
            }
          } else {
            return {
              ...test,
              status: 'failed',
              message: salaryResult.error || '급여 규칙 조회 실패',
              duration: Date.now() - startTime
            }
          }
  ...
  const categories = [
    ...
    { value: 'permissions', label: '권한', count: tests.filter(t => t.category === 'permissions').length },
    { value: 'integration', label: '통합', count: tests.filter(t => t.category === 'integration').length },
    { value: 'performance', label: '성능', count: tests.filter(t => t.category === 'performance').length }
  ]
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
        <div className="px-4 py-3 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
          <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">{testStats.total}</div>
        ...
        <div className="px-4 py-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
          <div className="text-2xl font-bold text-green-700 dark:text-green-400">{testStats.passed}</div>
        ...
        <div className="px-4 py-3 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
          <div className="text-2xl font-bold text-red-700 dark:text-red-400">{testStats.failed}</div>
        ...
        <div className="px-4 py-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
          <div className="text-2xl font-bold text-yellow-700 dark:text-yellow-400">{testStats.warning}</div>
        ...
        <div className="px-4 py-3 bg-gray-50 dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600">
          <div className="text-2xl font-bold text-gray-700 dark:text-gray-300">{testStats.pending}</div>
        ...
        <div className="px-4 py-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
          <div className="text-2xl font-bold text-blue-700 dark:text-blue-400">{testStats.running}</div>
