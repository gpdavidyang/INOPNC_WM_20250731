...
## Typography (타이포그래피) 가이드라인

### 폰트 크기 체계

#### 기본 폰트 크기
- **text-xs**: 12px → **text-sm**: 14px (최소 폰트 크기 개선)
...
#### 사용 가이드라인
- **최소 폰트 크기**: 14px (text-sm) - 가독성 확보
- **본문 텍스트**: 16px (text-base) 
- **제목**: 18px (text-lg) 이상
- **작은 부가 정보**: 14px (text-sm) - 기존 text-xs 대체

#### 컨텍스트별 적용
- **홈 화면 카드 내 작은 텍스트**: text-sm (14px)
- **버튼 텍스트**: text-sm (14px) 이상
- **설명 텍스트**: text-sm (14px)
- **레이블**: text-sm (14px)

#### 접근성 고려사항
- 최소 폰트 크기 14px 보장으로 시각적 접근성 향상
- 다크 모드에서도 동일한 크기 체계 적용
- 터치 모드에 따른 폰트 크기 조정 지원
